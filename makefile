...
cloud_run_deploy:
	gcloud run deploy pl-core-news-sm --image=${GCR_TAG} --max-instances=2 --min-instances=0 --port=5000 \
	--allow-unauthenticated --region=europe-west1 --memory=2Gi --cpu=4 -q

cloud_run_delete:
	gcloud run services delete pl-core-news-sm --region=europe-west1 -q
